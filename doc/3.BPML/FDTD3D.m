...
for i = 1:bpml
    sigEzx1(bpml+1-i) = sigExmax*((i*dx)/(bpml*dx))^m; % 左
    sigEzy1(bpml+1-i) = sigEymax*((i*dy)/(bpml*dy))^m; % 下
    sigEzx1(Nx+bpml+i) = sigExmax*((i*dx)/(bpml*dx))^m;	% 右
    sigEzy1(Ny+bpml+i) = sigEymax*((i*dy)/(bpml*dy))^m;	% 上
    sigEzx2(bpml+1-i) = sigExmax*((i*dx)/(bpml*dx))^m; % 左
    sigEzy2(bpml+1-i) = sigEymax*((i*dy)/(bpml*dy))^m; % 下
    sigEzx2(Nx+bpml+1+i) = sigExmax*((i*dx)/(bpml*dx))^m;	% 右
    sigEzy2(Ny+bpml+1+i) = sigEymax*((i*dy)/(bpml*dy))^m;	% 上
    sigHzx1(bpml+1-i) = sigHxmax*((i*dx)/(bpml*dx))^m; % 左
    sigHzy1(bpml+1-i) = sigHymax*((i*dy)/(bpml*dy))^m; % 下
    sigHzx1(Nx+bpml+i) = sigHxmax*((i*dx)/(bpml*dx))^m;	% 右
    sigHzy1(Ny+bpml+i) = sigHymax*((i*dy)/(bpml*dy))^m;	% 上
    sigHzx2(bpml+1-i) = sigHxmax*((i*dx)/(bpml*dx))^m; % 左
    sigHzy2(bpml+1-i) = sigHymax*((i*dy)/(bpml*dy))^m; % 下
    sigHzx2(Nx+bpml+1+i) = sigHxmax*((i*dx)/(bpml*dx))^m;	% 右
    sigHzy2(Ny+bpml+1+i) = sigHymax*((i*dy)/(bpml*dy))^m;	% 上
end
