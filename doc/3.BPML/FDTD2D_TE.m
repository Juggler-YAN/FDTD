...
for i = 1:bpml
    ...
    sigEx(Nx+bpml+i) = sigExmax*((i*dx)/(bpml*dx))^m;	% 右
    sigEy(Ny+bpml+i) = sigEymax*((i*dy)/(bpml*dy))^m;	% 上
    sigHx(bpml+1-i) = sigHxmax*((i*dx)/(bpml*dx))^m; % 左
    sigHy(bpml+1-i) = sigHymax*((i*dy)/(bpml*dy))^m; % 下
    sigHx(Nx+bpml+1+i) = sigHxmax*((i*dx)/(bpml*dx))^m;	% 右
    sigHy(Ny+bpml+1+i) = sigHymax*((i*dy)/(bpml*dy))^m;	% 上
end
